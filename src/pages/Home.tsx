...
interface CalendarReminder {
  title: string;
  time: string; // "HH:mm"
  triggeredDates?: string[]; // e.g., ["2025-08-07"]
}
...
function filterRemindersByTab(reminders: Reminder[]) {
  ...
  return reminders.filter((reminder) => {
    const mins = parseInterval(reminder.interval);
    const baseTime = reminder.lastTriggered || reminder.createdAt || Date.now();
    const nextDue = new Date(new Date(baseTime).getTime() + mins * 60 * 1000);
...
const checkCalendarReminders = () => {
  ...
  const now = new Date();
  const todayKey = now.toISOString().split('T')[0]; // e.g., "2025-08-07"
  ...
  const updatedReminders = reminders.map((reminder: CalendarReminder) => {
    ...
    if (!hasTriggered && reminder.time <= currentTime) {
      // Simple alert — you can replace this with triggerAlert(reminder)
  ...
  const checkReminders = () => {
    ...
    const updatedReminders = parsed.map((reminder: any) => {
  ...
  checkReminders(); // ✅ Run once on initial load

 const interval = setInterval(() => {
  checkReminders();           // ⏱️ existing interval logic
  checkCalendarReminders();   // 📅 new calendar-based reminders
...
<li className="border border-gray-600 p-4 sm:p-5 rounded-xl space-y-1">
              <strong className="text-lg">{reminder.title}</strong><br />
              <span className="text-sm text-gray-400">Interval: {reminder.interval} | Type: {reminder.type}</span><br />
